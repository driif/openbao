...
import (
	...
	// "encoding/json" // No longer directly needed in this file for forwardRequest
	"errors"
	"fmt"
	"net/http"
	"strconv"
	...
	// "github.com/openbao/openbao/sdk/v2/helper/wrapping" // No longer directly needed in forwardRequest
...
const (
	...
	// DefaultForwardingTimeout is the default timeout for forwarded requests.
	DefaultForwardingTimeout = 45 * time.Second
)
...
func (c *Core) switchedLockHandleRequest(httpCtx context.Context, req *logical.Request, doLocking bool) (resp *logical.Response, err error) {
	...
	if c.standby || c.perfStandby {
		...
		if c.perfStandby && isPerfStandbyEnabled && c.canProcessRequestLocally(req) {
			...
			c.logger.Debug("handling request locally on performance standby",
				"path", req.Path,
				"operation", req.Operation)
			// Continue with local processing below
		} else if c.perfStandby && isPerfStandbyEnabled {
			// Performance standby should forward this request
			c.logger.Debug("forwarding request from performance standby",
				"path", req.Path,
				"operation", req.Operation)
			return c.forwardRequest(httpCtx, req)
		} else if c.standby {
			// Regular standby cannot handle requests
			return nil, consts.ErrStandby
		}
		// If we get here, we're a performance standby handling locally
	}
...
// checkSSCTokenForward returns true if the SSC token should be forwarded from a standby to the active node.
// This function assumes that the token is an SSC token.
func (c *Core) checkSSCTokenForward(req *logical.Request) (bool, error) {
	// If SSC tokens are disabled, we can't forward them.
	if c.DisableSSCTokens() {
		return false, nil
	}
	// If the request doesn't have a client token, we can't forward it.
	if req.ClientToken == "" {
		return false, nil
	}

	// localAuthToken determines if this node *can* validate the token.
	// If localAuthToken returns false (e.g., on a performance standby, or if the token is old on an active node),
	// then this function returns true, indicating the token validation should be forwarded.
	// This ensures that performance standbys always forward SSC token validation,
	// and active nodes only validate "recent enough" tokens locally.
	return !c.localAuthToken(context.Background(), req.ClientToken), nil
}

// localAuthToken returns true if the token is considered "local enough" for this node to validate.
// The primary goal is to ensure performance standby nodes are conservative.
func (c *Core) localAuthToken(ctx context.Context, token string) bool {
	// Performance standby nodes (c.perfStandby == true) must always forward SSC token validation
	// to the active node. This is critical because the active node holds the sole authoritative
	// state for tokens, associated policies, entity aliases, and group memberships. Performance
	// standbys, by design, have a replicated state that might have a slight lag. Forwarding ensures
	// that all decisions involving SSC tokens are based on the most current and globally consistent
	// data, preventing potential security issues that could arise from validating against
	// (potentially) stale local data (e.g., a token that was just revoked on the active node
	// might still appear valid locally for a short window). This conservative approach prioritizes
	// consistency and correctness for all SSC token operations on performance standbys.
	// We use c.perfStandby directly as it reflects the current operational state set by HA logic.
	if c.perfStandby {
		return false
	}

	// If we don't have a token store (e.g., during early startup or certain error states),
	// we cannot determine if the token was created locally.
	if c.tokenStore == nil {
		return false
	}

	// For active nodes (c.HAStateWithLock() == consts.Active), a time-based heuristic is used.
	// The active node is the source of truth, so this check is less about consistency with another
	// node and more about whether the token information is likely to be fully settled within its
	// own data structures. If a token exists and was created very recently (within 5 seconds),
	// it's considered safe for the active node to validate it locally. This is a best-effort
	...
	if c.HAStateWithLock() == consts.Active {
		...
		// If the token was created within the last 5 seconds on the active node,
		// assume it's "local" for validation purposes.
		return time.Now().Unix()-entry.CreationTime < 5
	}

	// For traditional standbys (i.e., c.standby is true but c.perfStandby is false),
	// they are not designed for serving reads that require up-to-the-second consistency
	// for tokens. Similar to performance standbys, they should conservatively assume tokens
	// are not local to ensure that any sensitive operation relying on token validity
...
// If the token is not an SSC token, it will be returned as-is.
// If the token is an SSC token but it is disabled, it will return an error.
// If the token is an SSC token and it is enabled, it will check the HMAC and WAL index.
// If the HMAC is invalid, it will return an error.
// If the WAL index is not present, it will return an error that indicates the request should be forwarded.
// If the WAL index is present and valid, it will return the inner token.
