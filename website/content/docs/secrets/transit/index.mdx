...
as a source of random bytes. The transit engine can also act as an embedded
[KMIP server](./kmip-server), allowing external KMIP clients to create and use
transit-managed keys over a standard mTLS connection.
